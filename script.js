...
function handleDonate() {
    ...
    if (tg) {
        setTimeout(() => {
            tg.showAlert(
                `✅ Перенаправлено на оплату!\n\n` +
                `Після завершення оплати бонуси зарахуються автоматично протягом 1-2 хвилин.\n\n` +
                `Просто оплатіть - більше нічого робити не потрібно! 🎉`
            );
        }, 500);
    }

    // НЕ закриваємо Web App - користувач може повернутись
}
